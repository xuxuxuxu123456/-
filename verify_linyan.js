 ...
 * 默认角色「林彦」档案验证（原昵称：水色）
 *  ① 通讯录/首页昵称由「水色」改为「林彦」
 *  ② 编辑该联系人时，表单载入完整默认设定（含 兽人 属性）
 *  ③ 各真实 id（c_home_N / c_legacy_N / c_default_2 / 随机 id）下都能命中档案
 *  ④ 旧名「水色」未迁移时也能命中
 *  ⑤ 其余角色（埃洛温·影蚀 / 陆沉渊）档案不受影响
 *  ⑥ 新建模式仍保持空白
 *
 * 用法：node verify_linyan.js
 */
const fs = require('fs');
...
(async function main() {
  console.log('[A] 昵称改名：水色 → 林彦');
  {
    const s4 = fs.readFileSync('4_tongxun.html', 'utf8');
    ok(!/名称:\s*'水色'/.test(s4), '4页默认联系人已无「水色」');
    ok(/名称:\s*'林彦'/.test(s4), '4页默认联系人已改为「林彦」');
    const s1 = fs.readFileSync('1_shouyeyulan.html', 'utf8');
    ok(!/>水色</.test(s1), '1页会话列表已无「水色」');
    ok(/>林彦</.test(s1), '1页会话列表已显示「林彦」');
  }

  console.log('\n[B] 旧数据一次性改名迁移（三个映射共存）');
  {
    const 新索引 = await 跑4页([
      { id: 'c_home_0', 名称: '水色', 备注: '', 头像: '', 消息: '', 时间: '' },
    ...
    ok(!名.includes('水色') && !名.includes('恋痛症') && !名.includes('喵喵又咪咪'), '旧名已全部清除');
    ok(新索引.length === 3, '未产生重复条目（实际 ' + 新索引.length + ' 条）');
  }

  console.log('\n[C] 编辑林彦 → 载入完整默认设定');
  {
    const f = await 跑2页('c_home_0', '林彦');
    ok(f.标题 === '编辑联系人', '页面标题为「编辑联系人」');
    ok(f.昵称 === '林彦', '昵称 = 林彦（实际 "' + f.昵称 + '"）');
    ok(f.生日 === '2050-12-21', '生日 = 2050-12-21（实际 "' + f.生日 + '"）');
    ok(f.身高 === '195', '身高 = 195（实际 "' + f.身高 + '"）');
    ok(f.性别 === '男', '性别 = 男（实际 "' + f.性别 + '"）');
    ok(/蛮荒之境/.test(f.世界观) && /银狼部落/.test(f.世界观), '世界观含 蛮荒之境 / 银狼部落');
    ['普通兽人', '精锐战士', '领主级', '王级', '天启级'].forEach(级 => {
      ok(f.世界观.includes(级), '世界观含等级「' + 级 + '」');
    });
    ok(/天启降世/.test(f.人物信息), '人物信息含「天启降世」');
    ['三岁', '七岁', '十二岁那年', '十八岁', '二十二岁'].forEach(t => {
      ok(f.人物信息.includes(t), '人物信息含时间线「' + t + '」');
    });
    ok(/万兽王座/.test(f.人物信息), '人物信息含「万兽王座」');
    ok(/别在我的领地上乱跑/.test(f.人物信息), '口头禅1 已载入');
    ok(/收起你的獠牙/.test(f.人物信息), '口头禅2 已载入');
    ok(/吵死了。安静点。/.test(f.人物信息), '口头禅3 已载入');
    ok(/……过来。/.test(f.人物信息), '口头禅4 已载入');
    ok(/你，是我的。/.test(f.人物信息), '口头禅5 已载入');
    ['孤高冷漠', '野性难驯', '绝对强大', '极端占有欲'].forEach(签 => {
      ok(f.性格标签.some(t => t.includes(签)), '性格标签包含「' + 签 + '」');
    });
    ok(f.人物属性选中.includes('兽人'), '★ 人物属性 = 兽人（实际 ' + JSON.stringify(f.人物属性选中) + '）');
    ok(f.人物属性选中.length === 1, '仅选中「兽人」一项（未误选其他）');
  }

  console.log('\n[D] 各种真实 id 下都能命中档案');
  {
    for (const id of ['c_home_0', 'c_legacy_0', 'c_default_2', 'cx7y7y7']) {
      const f = await 跑2页(id, '林彦');
      ok(f.昵称 === '林彦' && f.世界观.length > 100, 'id=' + id + ' → 昵称+世界观已填入');
    }
  }

  console.log('\n[E] 旧名索引（未迁移）也能命中档案');
  {
    const f = await 跑2页('c_home_0', '水色');
    ok(f.昵称 === '林彦', '旧名「水色」→ 载入林彦档案');
    ok(f.世界观.length > 100 && f.人物属性选中.includes('兽人'), '旧名场景下世界观+兽人属性已填入');
  }

  console.log('\n[F] 其余角色档案不受影响');
  {
    const a = await 跑2页('c_home_1', '埃洛温·影蚀');
    ok(a.昵称 === '埃洛温·影蚀' && /艾诺拉/.test(a.世界观), '埃洛温·影蚀 档案正常（艾诺拉世界观）');
    ok(a.生日 === '2033-08-30', '埃洛温生日仍为 2033-08-30');
    const b = await 跑2页('c_home_2', '陆沉渊');
    ok(b.昵称 === '陆沉渊' && /北都·京华/.test(b.世界观), '陆沉渊 档案正常（双城世界观）');
    ok(b.生日 === '1998-11-11', '陆沉渊生日仍为 1998-11-11');
    ok(b.人物属性选中.includes('人类') && a.人物属性选中.includes('人类'), '两人属性仍为人类（未被兽人污染）');
  }
